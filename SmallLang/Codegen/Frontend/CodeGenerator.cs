...
public partial class CodeGenerator(SmallLangNode RootNode)
{
    ...
    internal void Cast<T>(T self, SmallLangType dstType)
        where T : IHasAttributeTypeOfExpression, ISmallLangNode => Cast(self, new GenericSmallLangType(dstType));
    ...
    internal int[] GetRegisters(GenericSmallLangType Type) => GetRegisters(Type.Size);
