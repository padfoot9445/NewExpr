...
internal static class ComparisonExpressionVisitor
{
    internal static void Visit(ComparisonExpressionNode Self, CodeGenerator Driver)
    {
        ...
        List<IExpressionNode> Expressions =
            [Self.Expression, .. Self.OperatorExpressionPairs.Select(x => x.Expression)];
        ...
        List<int> Registers =
            [ExpressionRegister, .. Expressions.Select(_ => Driver.GetRegisters(GreatestCommonType!).First())];
        var Width = GreatestCommonType.Size;

        var ResultRegisters = Enumerable.Range(0, Operators.Count)
            .Select(_ => Driver.GetRegisters(TypeData.Bool).First()).ToList();
