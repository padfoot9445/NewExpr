...
internal class VariableNameVisitor : BaseASTVisitor
{
    ...
    {
        if (node.Flatten().OfType<IHasAttributeVariableName>()
            .All(x => x.VariableName!.Name != PlaceholderVariableNameName)) return;

        Func<IdentifierNode, string> GetErrorMessage = self =>
            $"Identifier {self.Data.Lexeme} was not defined before use at Line {self.Data.Line}, Position {self.Data.Position}.";

        StringBuilder UltimateErrorMessage = new();
        foreach (var NullIdentifierNode in
                 node.Flatten().OfType<IHasAttributeVariableName>()
                     .Where(x => x.VariableName!.Name == PlaceholderVariableNameName))
            UltimateErrorMessage.AppendLine(NullIdentifierNode is IdentifierNode IDNode
                ? GetErrorMessage(IDNode)
                : $"NodeType {NullIdentifierNode.GetType()} had an identifier which was not recognized"); //should really only be IdentifierNodes here
    ...
    protected override ISmallLangNode VisitArgListElement(ISmallLangNode? Parent, ArgListElementNode self)
    {
        ...
        if (self.Identifier is not null) self.Identifier.VariableName = new VariableName(ArgumentLabelVariableName);
    ...
    {
        return self;
        //looplabel handles defining itself
    }
    ...
    protected override ISmallLangNode VisitFunctionCall(ISmallLangNode? Parent, FunctionCallNode self)
    {
        self.Identifier.VariableName =
            new VariableName(
                PlaceholderVariableNameName); //hack to signal that we are going to assign the function name later. This should never clash with an actual variablename because all Variablenames begin with Global.
        return self;
    }
    ...
    protected override ISmallLangNode VisitIdentifier(ISmallLangNode? Parent, IdentifierNode self)
    {
        ...
        if (self.VariableName is null && !self.Scope.IsDefined(self.Data.Lexeme))
            throw new ExpaException(
                $"Identifier {self.Data.Lexeme} was not defined before use at Line {self.Data.Line}, Position {self.Data.Position}.");

        if (self.Scope.IsDefined(self.Data.Lexeme))
        {
            self.VariableName = self.Scope.SearchName(self.Data.Lexeme);
        }
        else
        {
            Debug.Assert(self.VariableName is not null, self.Data.Lexeme);
            Debug.Assert(self.VariableName.Name == PlaceholderVariableNameName ||
                         self.VariableName.Name == ArgumentLabelVariableName);
        }
