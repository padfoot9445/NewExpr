...
namespace SmallLang.Parser.Lexing;
...
public sealed class Lexer(string input) : Scanner(input)
{
    ...
    protected override IEnumerable<(string, TokenType)> MCTTM => new[]
    {
        (";", TokenType.Semicolon),
    ...
    {
        var lexer = LexerBuilder.BuildLexer<TokenType>().Result;
        var tokens = lexer.Tokenize(input).Tokens.MainTokens().SkipLast(1);
        var rt = tokens.Select(x =>
            IToken.NewToken(x.TokenID, x.Value, x.Position.Index, x.StringWithoutQuotes, x.Position.Line)).ToArray();
