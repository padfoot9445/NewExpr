...
{
    static Lexer GetLexer(string input)
    {
        return new Lexer(input);
    }
    public static IEnumerable<(string, TokenType)> GetAllValidValues()
    {
        return (IEnumerable<(string, TokenType)>)(typeof(Lexer).GetProperty("MCTTM", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(GetLexer(""))!);
    }
    ...
    public void Lex__Valid_Combinations__Returns_Correct_Token_Sequence(
        [ValueSource(nameof(GetAllValidValues))] (string, TokenType) t1,
        [ValueSource(nameof(GetAllValidValues))] (string, TokenType) t2
    )
    {
    ...
    public void Multiline_Comments_Skips_Properly()
    {
        ...
        Assert.That(lexer.Scan().Select(t => t.TT).ToArray(), Is.EquivalentTo(new[] { TokenType.Number, TokenType.LogicalAnd }));
    }
    [Test]
